...
class Actuator(metaclass=abc.ABCMeta):
    ...
    def __init__(self, id=None, driver=None, on_message=None, loop=None):
...
ActuatorTuple = namedtuple("ActuatorTuple", "cls, driver, on_message, id, loop, fields")
...
class ActuatorFactory:
    ...
    @classmethod
    def _build_kwargs(cls, driver, on_message, id, loop, other_fields):
        kwargs = {}
    ...
    def from_tuple(cls, actuator_tuple):
        cls_name, driver, on_message, id, loop, other_fields = actuator_tuple
        cls_obj = domo.utils.find_class(_this_package, cls_name)
        kwargs = cls._build_kwargs(driver, on_message, id, loop, other_fields)
        obj = cls_obj(**kwargs)
        return obj
